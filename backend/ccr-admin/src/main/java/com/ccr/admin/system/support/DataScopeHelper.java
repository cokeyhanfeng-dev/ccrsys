 ...
 * </ul>
 * 机构范围统一按 org_code 编码前缀匹配(LIKE '前缀%'),登录时写入 Sa-Token session,
 * {@link com.ccr.common.datascope.DataScopeContext} 在 @DataScope 标注接口执行期间恢复,
 * 由 {@link com.ccr.common.datascope.CcrDataPermissionHandler} 在 SQL 层注入。
 */
@Component
public class DataScopeHelper {
    ...
    public DataScope compute(Long userId) {
        if (userId == null) {
            return new DataScope(DataScope.LEVEL_SELF, null, null);
        }
        CcrSysUser user = userMapper.selectById(userId);
        if (user == null || "1".equals(user.getDelFlag())) {
            return new DataScope(DataScope.LEVEL_SELF, null, userId);
        }
        return compute(user);
    }
    ...
    public DataScope compute(CcrSysUser user) {
        ...
        switch (role) {
            ...
            case "auditor":
            case "admin":
                return new DataScope(DataScope.LEVEL_ALL, null, user.getId());
            case "branch_manager":
                // 支行行长:本支行及下辖网点,按支行编码前缀匹配(§5.4)
                return new DataScope(DataScope.LEVEL_DEPT, resolveBranchCode(user.getOrgId()), user.getId());
            case "dept_gm":
            case "vice_president":
                // 部门总经理/分管行领导:本部门,按本部门编码前缀匹配
                return new DataScope(DataScope.LEVEL_DEPT, resolveOrgCode(user.getOrgId()), user.getId());
            default:
                return new DataScope(DataScope.LEVEL_SELF, null, user.getId());
        }
    }
