...
-- ---------- 产品目录(§8A.5①/§10.3.22):产品主数据,申请页产品下拉/LPR明细产品类型/权限矩阵/产品边界权威来源 ----------
CREATE TABLE IF NOT EXISTS `ccr_product` (
  `id`                  BIGINT       NOT NULL,
  `tenant_id`           VARCHAR(20)  NOT NULL DEFAULT '000000',
  `business_no`         VARCHAR(64)  NOT NULL,
  `org_id`              BIGINT       NOT NULL,
  `status`              VARCHAR(32)  NOT NULL COMMENT 'ENABLED/DISABLED(停用后新申请不可选)',
  `version_no`          INT          NOT NULL DEFAULT 1,
  `create_dept`         BIGINT       NULL,
  `create_by`           BIGINT       NOT NULL,
  `create_time`         DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `update_time`         DATETIME     NULL,
  `del_flag`            CHAR(1)      NOT NULL DEFAULT '0',
  `product_code`        VARCHAR(32)  NOT NULL COMMENT '产品编码(唯一,一经启用禁改)',
  `product_name`        VARCHAR(64)  NOT NULL COMMENT '产品名称',
  `business_big_type`   VARCHAR(16)  NOT NULL COMMENT 'LOAN/DEPOSIT',
  `product_category`    VARCHAR(32)  NULL COMMENT '产品线:对公贷款/个人经营性贷款/对公定期/协定/通知/银票保证金/信用证保证金等',
  `customer_type`       VARCHAR(16)  NULL COMMENT '适用客户类型:INDIVIDUAL/CORPORATE_SINGLE/GROUP',
  `currency`            VARCHAR(8)   NOT NULL DEFAULT 'CNY' COMMENT '币种',
  `default_min_rate`    DECIMAL(9,6) NULL COMMENT '产品默认利率下限(申请页默认值)',
  `default_max_rate`    DECIMAL(9,6) NULL COMMENT '产品默认利率上限',
  `default_min_term_months` INT      NULL COMMENT '默认最短期限(月)',
  `default_max_term_months` INT      NULL COMMENT '默认最长期限(月)',
  `effective_date`      DATETIME     NOT NULL COMMENT '生效日',
  `publish_by`          BIGINT       NULL COMMENT '发布人(双人复核)',
  `review_by`           BIGINT       NULL COMMENT '复核人',
  `publish_time`        DATETIME     NULL COMMENT '发布时间',
  ...
  KEY `idx_product_bigtype` (`business_big_type`),
  KEY `idx_product_category` (`product_category`),
  KEY `idx_product_status` (`status`)
) ENGINE=InnoDB COMMENT='ccr_product 产品目录(产品主数据,§8A.5①权威来源)';

-- ---------- 产品审批链路配置(§8A.5②/§10.3.23):产品↔审批链路,路由引擎读取替代硬编码;支行行长节点恒必经(B13)不含开关 ----------
CREATE TABLE IF NOT EXISTS `ccr_product_route` (
  `id`                  BIGINT       NOT NULL,
  `tenant_id`           VARCHAR(20)  NOT NULL DEFAULT '000000',
  `business_no`         VARCHAR(64)  NOT NULL,
  `org_id`              BIGINT       NOT NULL,
  `status`              VARCHAR(32)  NOT NULL COMMENT 'DRAFT/PENDING_REVIEW/PUBLISHED/OBSOLETE(§8A.2统一状态机)',
  `version_no`          INT          NOT NULL DEFAULT 1,
  `create_dept`         BIGINT       NULL,
  `create_by`           BIGINT       NOT NULL,
  `create_time`         DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `update_time`         DATETIME     NULL,
  `del_flag`            CHAR(1)      NOT NULL DEFAULT '0',
  `product_code`        VARCHAR(32)  NOT NULL COMMENT '关联产品目录(启用产品)',
  `business_big_type`   VARCHAR(16)  NOT NULL COMMENT 'LOAN/DEPOSIT(冗余,用于过滤)',
  `flow_key`            VARCHAR(64)  NULL COMMENT 'Warm-Flow流程定义key(空=默认流程)',
  `start_node_code`     VARCHAR(32)  NULL COMMENT '起始节点(route_mode=DIRECT_VOTE时置空)',
  `route_mode`          VARCHAR(16)  NOT NULL COMMENT 'CHAINED链式逐级/DIRECT_VOTE直接上会(D16b存款/保证金)',
  `mandatory_vote`      CHAR(1)      NOT NULL DEFAULT 'N' COMMENT '是否强制六人小组(Y/N,如对公贷款>5000万)',
  `president_decision`  CHAR(1)      NOT NULL DEFAULT 'N' COMMENT '是否必经总行行长决策(Y/N)',
  `vote_condition`      JSON         NULL COMMENT '上会条件(amount_tier/enterprise_type/利率越界等,命中即上会)',
  `priority`            INT          NOT NULL DEFAULT 0 COMMENT '同一产品多配置时排序(低值优先)',
  `effective_date`      DATETIME     NOT NULL COMMENT '生效日(同产品仅一版生效)',
  `publish_by`          BIGINT       NULL COMMENT '发布人(双人复核)',
  `review_by`           BIGINT       NULL COMMENT '复核人',
  `publish_time`        DATETIME     NULL COMMENT '发布时间',
  `remark`              VARCHAR(255) NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_product_route` (`product_code`,`effective_date`),
  KEY `idx_route_product` (`product_code`),
  KEY `idx_route_status` (`status`)
) ENGINE=InnoDB COMMENT='ccr_product_route 产品审批链路配置(§8A.5②,路由引擎读取替代硬编码)';

-- ---------- LPR 明细(§8A.3/§10.3.17:按指标×产品逐行,替换单行两列) ----------
CREATE TABLE IF NOT EXISTS `ccr_lpr_config` (
  `id`             BIGINT       NOT NULL,
  `tenant_id`      VARCHAR(20)  NOT NULL DEFAULT '000000',
  `business_no`    VARCHAR(64)  NOT NULL,
  `org_id`         BIGINT       NOT NULL,
  ...
  `update_time`    DATETIME     NULL,
  `del_flag`       CHAR(1)      NOT NULL DEFAULT '0',
  `version_id`     BIGINT       NOT NULL COMMENT 'LPR版本主键(ccr_lpr_version.id)',
  `lpr_term`       VARCHAR(8)   NOT NULL COMMENT '指标/期限:1Y / 5Y+',
  `product_type`   VARCHAR(32)  NOT NULL COMMENT '产品类型(对应产品编码 ccr_product.product_code,权威来源 §8A.5①)',
  `lpr_value`      DECIMAL(9,6) NOT NULL COMMENT 'LPR值(%百分数,0.5–8,0.05整数倍)',
  `lpr_bp`         DECIMAL(9,2) NULL COMMENT '相对LPR加点BP(可空,按产品差异化阈值)',
  `remark`         VARCHAR(255) NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_lpr_config` (`version_id`,`lpr_term`,`product_type`),
  KEY `idx_lpr_config_term` (`lpr_term`,`product_type`)
) ENGINE=InnoDB COMMENT='ccr_lpr_config LPR明细(按指标×产品,§8A.3)';
