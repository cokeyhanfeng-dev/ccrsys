...
菜单与角色调整后，可运行 `./dev navigation-api-test` 对 `ccrsys-test` 执行真实 HTTP 回归：覆盖授权生效、管理员边界、三级目录、并发成环拒绝及 SQL 重复执行。脚本仅连接本机 18080 端口，生成虚构账号并在结束时恢复菜单/角色配置、删除临时记录、注销测试会话。详见 [开发与测试环境](docs/11_开发与测试环境.md)。
