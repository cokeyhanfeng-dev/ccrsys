...
## 集团客户数仓数据集(9 张,V1.0 A.5)

> 依据:`db/02_external_data.sql` 195-374 行(dw_*_snapshot 集团段)。集团功能数据源,层级:**集团-成员-集团授信-成员额度-用信分项-贷款合同-借据**。
...
| T16 | `dw_credit_tranche_snapshot` | 用信分项快照 | 分项 |
| T17 | `dw_loan_contract_snapshot` | 贷款合同快照(决议核验利率源) | 合同 |
| T18 | `dw_loan_note_snapshot` | 借据快照(决议核验比对合同利率) | 借据 |
| T19 | `dw_deposit_account_snapshot` | 存款账户快照(账号反查,哈希匹配) | 账户 |
...
## T16 `dw_credit_tranche_snapshot` 用信分项快照

```sql
CREATE TABLE dw_credit_tranche_snapshot (
  etl_md5             BIGINT       NOT NULL AUTO_INCREMENT,
  data_dt             DATE         NOT NULL COMMENT '数据日期(DATA_DT)',
...
## T17 `dw_loan_contract_snapshot` 贷款合同快照
...
| T16 | `dw_credit_tranche_snapshot` | 用信/担保分项 | 每日 |
| T17 | `dw_loan_contract_snapshot` | 贷款合同 | 每日 |
| T18 | `dw_loan_note_snapshot` | 借据（核验用） | 每日 |
| T19 | `dw_deposit_account_snapshot` | 存款账户（密文+哈希） | 每日 |
