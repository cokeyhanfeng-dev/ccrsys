...
OA 如需由中转服务先登录再跳转页面，使用 [Flask 中转示例](oa-relay/README.md)，移动端已支持 CCR 系统 token 回跳；原 ticket/oaToken 入口保留。

### 发布包查找性能（2026-09-17）

自动查包只匹配查包目录第一层的 `ccr-release-*.tar.gz`，先按修改时间从新到旧排序，再逐个读取包内组件，首个匹配即停止；显示当前检查包名和候选进度。保留包内产物判断、部署前确认、完整性校验及备份。可显式指定包路径跳过候选查找。
